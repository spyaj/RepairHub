...
import { likeCommunityThread } from "@/lib/server/community/service";
...
export async function POST(req: NextRequest, context: RouteContext) {
  ...
  try {
    const result = await likeCommunityThread(session.user.id, parsedSlug.data);
    ...
    if (result.alreadyLiked) {
      return errorResponse(409, "ALREADY_LIKED", "You already liked this discussion", requestId);
    }

    log("info", "Community thread liked", {
      requestId,
      route: "/api/community/forum/[slug]/like",
    ...
    log("error", "Community thread like failed", {
      requestId,
      route: "/api/community/forum/[slug]/like",
      actorId: session.user.id,
      role: session.user.role,
