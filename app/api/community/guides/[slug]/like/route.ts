...
import { likeCommunityGuide } from "@/lib/server/community/service";
...
export async function POST(req: NextRequest, context: RouteContext) {
  ...
  try {
    const result = await likeCommunityGuide(session.user.id, parsedSlug.data);
    ...
    if (result.alreadyLiked) {
      return errorResponse(409, "ALREADY_LIKED", "You already liked this guide", requestId);
    }

    log("info", "Community guide liked", {
      requestId,
      route: "/api/community/guides/[slug]/like",
    ...
    log("error", "Community guide like failed", {
      requestId,
      route: "/api/community/guides/[slug]/like",
      actorId: session.user.id,
      role: session.user.role,
